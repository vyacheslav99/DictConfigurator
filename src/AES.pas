...
function DecryptAES(Source, Key: string; KeySizeMode: TAESKeySize): string;
...
procedure EncryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey128; var OutBuf: TAESBuffer); overload;
procedure EncryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey192; var OutBuf: TAESBuffer); overload;
procedure EncryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey256; var OutBuf: TAESBuffer); overload;
...
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey128;
...
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey192;
  Dest: TStream); overload;

procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey256; Dest: TStream); overload;
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey256;
  Dest: TStream); overload;

// Stream encryption routines (CBC mode)
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal; const Key: TAESKey128;
  const InitVector: TAESBuffer; Dest: TStream); overload;
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey128;
  const InitVector: TAESBuffer; Dest: TStream); overload;

procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal; const Key: TAESKey192;
  const InitVector: TAESBuffer; Dest: TStream); overload;
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey192;
  const InitVector: TAESBuffer; Dest: TStream); overload;

procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal; const Key: TAESKey256;
  const InitVector: TAESBuffer; Dest: TStream); overload;
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey256;
  const InitVector: TAESBuffer; Dest: TStream); overload;
...
procedure DecryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey128; var OutBuf: TAESBuffer); overload;
procedure DecryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey192; var OutBuf: TAESBuffer); overload;
procedure DecryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey256; var OutBuf: TAESBuffer); overload;
...
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey128;
  Dest: TStream); overload;

procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey192; Dest: TStream); overload;
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey192;
  Dest: TStream); overload;

procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey256; Dest: TStream); overload;
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey256;
  Dest: TStream); overload;

// Stream decryption routines (CBC mode)
procedure DecryptAESStreamCBC(Source: TStream; Count: cardinal; const Key: TAESKey128;
  const InitVector: TAESBuffer; Dest: TStream); overload;
procedure DecryptAESStreamCBC(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey128;
  const InitVector: TAESBuffer; Dest: TStream); overload;

procedure DecryptAESStreamCBC(Source: TStream; Count: cardinal; const Key: TAESKey192;
  const InitVector: TAESBuffer; Dest: TStream); overload;
procedure DecryptAESStreamCBC(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey192;
  const InitVector: TAESBuffer; Dest: TStream); overload;

procedure DecryptAESStreamCBC(Source: TStream; Count: cardinal; const Key: TAESKey256;
  const InitVector: TAESBuffer; Dest: TStream); overload;
procedure DecryptAESStreamCBC(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey256;
  const InitVector: TAESBuffer; Dest: TStream); overload;
...
procedure EncryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey128; var OutBuf: TAESBuffer);
var
...
procedure EncryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey192; var OutBuf: TAESBuffer);
var
  T0, T1: array [0..3] of longword;
  W0, W1, W2, W3: longword;
...
procedure EncryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey256; var OutBuf: TAESBuffer);
var
  T0, T1: array [0..3] of longword;
  W0, W1, W2, W3: longword;
...
procedure DecryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey128; var OutBuf: TAESBuffer);
var
  T0, T1: array [0..3] of longword;
  W0, W1, W2, W3: longword;
...
procedure DecryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey192; var OutBuf: TAESBuffer);
var
  T0, T1: array [0..3] of longword;
  W0, W1, W2, W3: longword;
...
procedure DecryptAESBlock(const InBuf: TAESBuffer; const Key: TAESExpandedKey256; var OutBuf: TAESBuffer);
var
  T0, T1: array [0..3] of longword;
  W0, W1, W2, W3: longword;
...
begin
  ...
  begin
    ...
    EncryptAESBlock(TempIn, ExpandedKey, TempOut);
...
begin
  ...
  begin
    ...
    if Done < SizeOf(TempIn) then
      raise EStreamError.Create(SReadError);
    DecryptAESBlock(TempIn, ExpandedKey, TempOut);
    Done := Dest.Write(TempOut, SizeOf(TempOut));
    if Done < SizeOf(TempOut) then
      raise EStreamError.Create(SWriteError);
    Dec(Count, SizeOf(TAESBuffer));
...
// Stream encryption routines (CBC mode)
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal; const Key: TAESKey128;
  const InitVector: TAESBuffer; Dest: TStream);
var
  ExpandedKey: TAESExpandedKey128;
...
begin
  ...
  case KeySizeMode of
    ...
    begin
      ...
      Move(PChar(Key)^, Key192, Min(SizeOf(Key192), Length(Key)));
      EncryptAESStreamECB(Source, 0, Key192, Dest);
    end;
    ak256:
    begin
      FillChar(Key256, SizeOf(Key256), 0);
      Move(PChar(Key)^, Key256, Min(SizeOf(Key256), Length(Key)));
...
begin
  Source.ReadBuffer(sz, SizeOf(sz));
  ...
  case KeySizeMode of
    ...
    begin
      FillChar(Key192, SizeOf(Key192), 0);
      Move(PChar(Key)^, Key192, Min(SizeOf(Key192), Length(Key)));
      DecryptAESStreamECB(Source, Source.Size - Source.Position, Key192, Dest);
    end;
    ak256:
    begin
      FillChar(Key256, SizeOf(Key256), 0);
      Move(PChar(Key)^, Key256, Min(SizeOf(Key256), Length(Key)));
      DecryptAESStreamECB(Source, Source.Size - Source.Position, Key256, Dest);
    end;
  end;
...
begin
  Src := TStringStream.Create(Source);
  Dst := TStringStream.Create('');

  try
    EncryptAESStream(Dst, Src, Key, KeySizeMode);
...
begin
  ...
  try
    DecryptAESStream(Dst, Src, Key, KeySizeMode);
    result := Dst.DataString;
  finally
    Src.Free;
    Dst.Free;
