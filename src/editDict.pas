  ...
  try
    ...
    begin
      ...
      begin
        ...
        gcsGenInsertSQL(Script, sl, tableName, 'REFERENCE_PK, ROLE_PK, ADD_, EDIT_, DEL_, VIEW_REF, VIEW_, EXPORT, IMPORT, FILTRING, GRID_SAVE, GUID',
          [], []);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if (dsDict.FieldByName(keyFieldName).AsInteger <> -1) or (not isStartForm) then
        begin
          result := true;
          Script.Add('');
          Script.Add('  ' + comment);
...
begin
  ...
  case Mode of
    ...
    begin
      ...
      // пишем в лог
      FMain.AddToRefLog(cotDict, edDictDescriptor.Text, VarToStr(Properties.Guid), rltCreate);
    end;
    omEdit:
    begin
      // сначала проверим - если нет ни одного объекта БД, то создавать нельзя
      ...
      // пишем в лог
      FMain.AddToRefLog(cotDict, edDictDescriptor.Text, VarToStr(Properties.Guid), rltUpdate);
    end
    else result := false;
  end;
...
begin
  ...
  if VarIsNull(TmplPk) or (TmplPk = 1) or (TmplPk = 0) or Insert then
    ...
    FMain.AddToRefLog(cotDict, Properties.Descriptor, VarToStr(Properties.Guid), rltUpdate, 'Изменение текста шаблона прав ' + VarToStr(TmplPk));
  end;
