...
type
  ...
  private
    ...
    procedure GenChangesSQL(Script, Vars: TStringList);
...
begin
  if VarIsNull(Value) and Field.IsNull then
    arr.Add('    ' + Field.FieldName + ' = null')
  else if Field.Value <> Value then
    arr.Add('    ' + Field.AsString + ' = ' + VariantToDBStr(Value, QuotedString, EmptyStrToNull, ZeroToNull, QuoteSymb));
end;
...
procedure TFEditDict.GenChangesSQL(Script, Vars: TStringList);
var
  sl: TStringList;
  i: integer;
...
begin
  ...
  try
    ...
    Script.Add('  select PK, MAIN_FORM_PK from DYNAMIC_FORM_REFERENCE where DESCRIPTOR_ = ''' + dsDictDESCRIPTOR_.AsString + ''' into :REF_PK, :MAIN_FORM_PK^');
    ...
    if sl.Count > 0 then
    begin
      sl.Add('    MODIFY = current_timestamp');
      Script.Add('');
      Script.Add('  /* DYNAMIC_FORM */');
      Script.Add('  update DYNAMIC_FORM set');
      Script.Add(TextToString(sl.Text, '', ','#13#10));
      Script.Add('  where PK = :MAIN_FORM_PK^');
    end;
